        ...
        <div class="col-md-12">
            ...
            <form action="{{ route('products.update', $product->id) }}" method="POST">
                ...
                <div class="form-group @if($errors->has('name')) has-error @endif">
                    ...
                    <input type="text" id="value-field" name="value" class="form-control" value="{{ is_null(old("value")) ? $product->value : old("value") }}"/>
                       @if($errors->has("value"))
                    ...
                    <input type="text" id="amount-field" name="amount" class="form-control" value="{{ is_null(old("amount")) ? $product->amount : old("amount") }}"/>
                    ...
                    <select id="artisan_id-field" name="artisan_id" class="form-control">
                      @foreach ($artisans as $artisan)
                        @if($artisan->id == $product->artisan->id)
                          <option selected="selected" value={{ $artisan->id }}>{{ $artisan->name }}</option>
                        @else
